                ...
                <Box display="flex" justifyContent="space-between" marginBottom="1rem" padding="1rem 1.8rem 0">
                    ...
                    <Box display="flex" gap="1.4rem" alignItems="center">
                        ...
                        <Box display="flex">
                            ...
                            <IconButton ><NavigateBeforeIcon sx={{ border: "1px solid #CED0DA", borderRadius: "3px", width: "2rem", background: "linear - gradient(0deg, #F2F4F7 0 %, #FFFFFF 100 %)" }} /></IconButton>
                            <IconButton><NavigateNextIcon sx={{ border: "1px solid #CED0DA", borderRadius: "3px", width: "2rem" , background: "linear - gradient(0deg, #F2F4F7 0 %, #FFFFFF 100 %)" }}/></IconButton>
                        </Box>
                    </Box>
                </Box>
                ...
                <Box sx={{ display: "flex", flexDirection: "column", gap: ".2rem" }} >
                    {chart.map((item) => (
                        <Card sx={{ padding: "1.5rem 2.8rem 1.9rem 1.8rem", display: "flex", flexDirection: "column", gap: "0rem" }}>
                            <Typography sx={{ fontWeight: "bold" }} marginBottom=".6rem">{item.header}</Typography>
                            <img src={bar} alt="" />
                            <Box marginTop=".5rem" display="flex" flexDirection="column" gap=".4rem">

                                <Typography fontSize="12px" color="#262626">Pending Orders: {item.pendingOrders}</Typography>
                                <Typography fontSize="12px" color="#262626">Reconcilled Orders: {item.reconcilledOrders}</Typography>
                                <Typography fontSize="12px" color="#262626">Total Orders:{item.total}</Typography>
                            </Box>
                        </Card>
                    ))}
